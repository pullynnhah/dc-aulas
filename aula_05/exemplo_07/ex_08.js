...
function checkoutDiscount(name, purchase, isNewClient, isCashPayment) {
  let discount = 0;
  if (isNewClient) discount += 10;
  if (isCashPayment) discount += 10;
  if (purchase >= 1000) discount += 10;
  else if (purchase >= 500) discount += 5;

  console.log(`Thanks, ${name} for buying with us!`);
  console.log(`Total: $${purchase.toFixed(2)}`);

  if (discount === 0) {
    const coupon = Math.floor(Math.random() * 10) + 10;
    console.log(`Here is your coupon for next purchase of ${coupon}%`);
  } else {
    const finalCost = purchase * (1 - discount / 100);
    console.log(`You have a discount of ${discount}%`);
    console.log(`Final cost: $${finalCost.toFixed(2)}`);
  }
}

checkoutDiscount("Paula", 1000, true, true);
console.log();

checkoutDiscount("Luna", 100, false, false);
